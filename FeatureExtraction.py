...
def makeCSV():
    ...
    if not(os.path.exists('DataSet\\Features/Training')):
        os.makedirs('DataSet\\Features/Training')
        print('New folder "Features/Training" created')
    if not(os.path.exists('DataSet\\Features/Testing')):
        os.makedirs('DataSet\\Features/Testing')
    ...
    for person in range(1,16):
        ...
        with open('DataSet\\Features\\Training/training_'+per+'.csv', 'w') as handle:
            ...
            # Training set
            print("train 0-3 for 1")
            for i in range(0,3):
                source = os.path.join(gpath, per+per+'_00'+str(i)+'.png')
                features = getCSVFeatures(path=source)
                handle.write(','.join(map(str, features))+',1\n')
            print("train 0-3 for 0")
            for i in range(0,3):
